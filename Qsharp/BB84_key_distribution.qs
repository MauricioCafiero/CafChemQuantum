namespace TestState {
    ...
    operation distribute_key(num_trips: Int, trip_size: Int, prob_eavesdropping: Double) : Unit {
        ...
        for trip in 0..num_trips-1 {
            ...
            // Choose sender measurement bases randomly
            let sender_bases = generate_random_bits(trip_size);
            
            // Choose sender key bits
            let sender_values = generate_random_bits(trip_size);

            // sender encode values
            encode_qubits(sender_values, sender_bases, qubits);
            ...
            let (sender_trip_result, receiver_trip_result) 
                = compare_bases(sender_bases, sender_values, receiver_bases, receiver_results);
    ...
    operation encode_qubits(bit_values: Bool[], bases: Bool[], qubits: Qubit[]) : Unit {
    ...
    function compare_bases(sender_bases: Bool[], sender_values: Bool[], 
